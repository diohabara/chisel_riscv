...
class Core extends Module {
  ...
  val jmp_flg   = (inst === JAL)
  val alu_out   = Wire(UInt(WORD_LEN.W))
  // Q: MuxCaseってなんだっけ？
  // A: 条件と出力が複数ある場合に使う。
  //    ここでは、br_flgがtrueの場合はbr_targetを、jmp_flgの場合はalu_outを、instがECALLの場合はcsr_regfile(0x305)を、それ以外の場合はpc_plus4を出力する。
  val pc_next = MuxCase(
    pc_plus4,
    Seq(
      br_flg  -> br_target,
  ...
  val imm_b =
    Cat(inst(31), inst(7), inst(30, 25), inst(11, 8))
  val imm_b_sext = Cat(Fill(19, imm_b(11)), imm_b, 0.U(1.U))
  val imm_j =
    Cat(inst(31), inst(19, 12), inst(20), inst(30, 21))
  ...
  // Q: ListLookup?
  // A: ListLookup(inst, default, mapping)
  //    returns matched List from mapping if inst matches with BitPat
  ...
  alu_out := MuxCase(
    ...
    Seq(
      ...
      (exe_fun === ALU_SRA) -> (op1_data
        .asSInt() >> op2_data(4, 0)).asUInt(),
      (exe_fun === ALU_SLT) -> (op1_data.asSInt() < op2_data
        .asSInt()).asUInt(),
  ...
  br_flg := MuxCase(
    ...
    Seq(
      ...
      (exe_fun === BR_BLT) -> (op1_data.asSInt() < op2_data
        .asSInt()),
      (exe_fun === BR_BGE) -> !(op1_data.asSInt() < op2_data
        .asSInt()),
